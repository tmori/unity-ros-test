...
#include "robotics_demo/Actuator.h"
#include "robotics_demo/Sensor.h"
#include <stdio.h>
...
ER ev3_motor_stop(int id, bool pwr)
{
    ev3_motor_set_power(id, 0);
    return 0;
}
static ER ev3_motor_steer(int id1, int id2, int power, int t)
{
...
static int16_t ev3_ultrasonic_sensor_get_distance(int id)
{
    return sensing_data.ultrasonic / 10;
}

static colorid_t ev3_color_sensor_get_color(int id)
{
    return sensing_data.color;
}
typedef unsigned char uint8_t;
static uint8_t ev3_color_sensor_get_reflect(int id)
{
    return sensing_data.reflect;
}
...
static void sensorCallback(const robotics_demo::Sensor::ConstPtr& msg)
{
    //ROS_INFO("I heard: [%d]", msg->sensor_color0);
    sensing_data.ultrasonic = msg->sensor_ultrasonic;
    sensing_data.color = (colorid_t)msg->sensor_color0;
    sensing_data.touch[0] = msg->touch_sensor0;
    sensing_data.touch[1] = msg->touch_sensor1;
    return;
}
/*
 * 1.1 走る(前進操作)
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        ros_actuator_data.led = actuator_data.led;
        ros_actuator_data.motor_power_a = actuator_data.motor_power[0];
        ros_actuator_data.motor_power_b = actuator_data.motor_power[1];
        ros_actuator_data.motor_power_c = actuator_data.motor_power[2];
        actuator_pub.publish(ros_actuator_data);
        ++count;
    }
    return 0;
}